...
from src.programs_utils import create_program_layout, create_program_callbacks
...
alerts = [
    dbc.Alert(
        ...
        color="danger",
        id=f"{ID_PREFIX}-manual-text-form-market-alert",
        dismissable=True,
        is_open=False,
    ),
    dbc.Alert(
        ...
        color="danger",
        id=f"{ID_PREFIX}-manual-text-form-std-unit-alert",
        dismissable=True,
        is_open=False,
    ),
    dbc.Alert(
        ...
        color="danger",
        id=f"{ID_PREFIX}-manual-text-program-start-alert",
        dismissable=True,
        is_open=False,
    ),
    dbc.Alert(
        "Percentage value exceeding 100. Please change it if it's not intentional.",
        id=f"{ID_PREFIX}-manual-text-form-percentage-value-alert",
        dismissable=True,
        is_open=False,
        color="danger",
    )
...
program_button, program_modal, programs_table, program_details_modal = create_program_layout(f"{ID_PREFIX}-manual", "text")


deleted_sample_alert_modal = dbc.Modal(
        [
            dbc.ModalHeader(dbc.ModalTitle("Program Exists for Sample")),
            dbc.ModalBody(id=f"{ID_PREFIX}-manual-text-form-deleted-alert-modal-body"),
        ],
        is_open=False,
        id=f"{ID_PREFIX}-manual-text-form-deleted-alert-modal",
    )
...
create_program_callbacks(id_prefix=f"{ID_PREFIX}-manual", form_type="text", inputs=inputs)

# * ------------------------------- Enable Percentage Form Fields --------------------------------
percentage_input_ids = ["percentage-type", "base-year", "base-scope"]
@callback(
    [Output(f"{ID_PREFIX}-manual-{i}-text-input", "disabled") for i in percentage_input_ids],
    [Output(f"{ID_PREFIX}-manual-{i}-text-input", "value") for i in percentage_input_ids],
    Input(f"{ID_PREFIX}-manual-std-unit-text-input", "value"),
    [Input(f"{ID_PREFIX}-manual-{i}-text-input", "value") for i in percentage_input_ids],
)
...
@callback(
    Output(f"{ID_PREFIX}-manual-text-form-percentage-value-alert", "is_open"),
    Input(f"{ID_PREFIX}-manual-value-text-input", "value"),
    Input(f"{ID_PREFIX}-manual-std-unit-text-input", "value")
)
...
@callback(
    Output(f"{ID_PREFIX}-manual-market-text-input", "valid"),
    Output(f"{ID_PREFIX}-manual-text-form-market-alert", "is_open"),
    Input(f"{ID_PREFIX}-manual-market-text-input", "value"),
    State(f"{ID_PREFIX}-manual-scope-text-input", "value"),
)
...
@callback(
    Output(f"{ID_PREFIX}-manual-std-unit-text-input", "valid"),
    Output(f"{ID_PREFIX}-manual-text-form-std-unit-alert", "is_open"),
    Input(f"{ID_PREFIX}-manual-std-unit-text-input", "value"),
    State(f"{ID_PREFIX}-manual-raw-unit-text-input", "value"),
)
...
@callback(
    ...
    Input(f"{ID_PREFIX}-manual-market-text-input", "valid"),
    Input(f"{ID_PREFIX}-manual-std-unit-text-input", "valid"),
)
def disable_submit(
    year,
    scope,
...
@callback(
    ...
    Output(f"{ID_PREFIX}-manual-text-form-deleted-alert-modal", "is_open"),
    Output(f"{ID_PREFIX}-manual-text-form-deleted-alert-modal-body", "children"),
...
@callback(
    Output(f"{ID_PREFIX}-manual-text-form-program-details-modal-body", "children"),
    Output(f"{ID_PREFIX}-manual-text-form-program-details-modal", "is_open"),
    Input(f"{ID_PREFIX}-manual-programs-text-container", "active_cell"),
    State(f"{ID_PREFIX}-manual-samples-text-container", "data"),
    State(f"{ID_PREFIX}-manual-programs-text-container", "data"),
...
def display_program_details_modal(active_cell, samples, programs, data_store):
    if active_cell:
        ...
        for sample in program_samples:
            ...
            page_number = sample["Page Number Evidence"]
            ...
            sample_form = dbc.Form(form_with_answers + evidence_inputs)
